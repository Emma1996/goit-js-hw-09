...
const onFormSubmit = ()=>{
    ...
    for(let i = 0; i < amountValue; i++){
        ...
        let delay2 = 0;
        if (i == 0) delay2 = Number.parseInt(firstDelay);
        else delay2 = Number.parseInt(firstDelay + stepDelayValue * i);
        createPromise(i, delay2).then(({ position , delay: delay3  })=>(0, _notiflixDefault.default).Notify.success(`Fulfilled promise ${position} in ${delay3}ms`)).catch(({ position , delay: delay4  })=>(0, _notiflixDefault.default).Notify.failure(`Rejected promise ${position} in ${delay4}ms`));
    }
};
